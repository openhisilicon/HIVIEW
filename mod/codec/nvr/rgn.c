...
int gsf_rgn_osd_set_ex(int ch, int idx, gsf_osd_t* osds, int osd_count)
{
	...
	for (i = 0; i < GSF_CODEC_VENC_NUM; i++)
	{
		...
		{
			info->osdW = 1920;//(osd->wh[0] > info->osdW) ? ALIGN_UP(osd->wh[0], 2) : info->osdW;
			info->osdH = 1080;//(osd->wh[1] > info->osdH) ? ALIGN_UP(osd->wh[1], 2) : info->osdH;
		}

		rgn_obj[handle].rgn.stRegion.unAttr.stOverlay.stSize.u32Width = 1920;// ALIGN_UP(info->osdW, 2);
		rgn_obj[handle].rgn.stRegion.unAttr.stOverlay.stSize.u32Height = 1080;// ALIGN_UP(info->osdH, 2);
		...
		/*	int l = 0;
			...
			if ((osd->wh[0] && osd->wh[1]))
			{
				...
				for (size_t i = 0; i < osd_count; i++)
				{
					...
					int _width = osd_t->wh[0], _height = osd_t->wh[1];


					for (j = 0; j < _width; j++)
					{
						p[j + (0 + _y) * info->osdW + _x]
							= p[j + (1 + _y) * info->osdW + _x]
							= p[j + (2 + _y) * info->osdW + _x]
					...
					for (j = 0; j < _height; j++)
					{
						p[(j + _y) * info->osdW + 0 + _x]
							= p[(j + _y) * info->osdW + 1 + _x]
							= p[(j + _y) * info->osdW + 2 + _x]
