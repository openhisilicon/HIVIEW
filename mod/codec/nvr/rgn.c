...
static int codec_venc_width(int ch, int i)
{
  ...
  return codec_nvr.venc[i].width;
}

static int codec_venc_height(int ch, int i)
{

  return codec_nvr.venc[i].height;
}
...
int gsf_rgn_canvas(int ch, int idx, gsf_osd_act_t *act)
{
  ...
  if(act->w != codec_nvr.venc[sub].width)
  {
    //warn("act->w:%d, codec_nvr width:%d\n", act->w, codec_nvr.venc[sub].width);
    return -1;
  }
  
  //printf("act->w:%d, codec_nvr width:%d\n", act->w, codec_nvr.venc[sub].width);
