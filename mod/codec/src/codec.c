...
int main(int argc, char *argv[])
{
    ...
    if(jscon_parm_load(argv[1], &codec_parm) < 0)
    {
      jscon_parm_save(argv[1], &codec_parm);
      jscon_parm_load(argv[1], &codec_parm);
    }
    info("parm.venc[0].vst[0].type:%d\n", codec_parm.venc[0].vst[0].type);
