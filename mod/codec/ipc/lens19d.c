...
int lens19d_uart_write(unsigned char *buf, int size)
{
  int ret = 0;
...
int lens19d_uart_open(char *ttyAMA, int baudrate)
{
...
int (*gsf_uart_open)(char *ttyAMA, int baudrate) = lens19d_uart_open;
int (*gsf_uart_write)(unsigned char *buf, int size) = lens19d_uart_write;
