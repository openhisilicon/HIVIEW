    ...
    hi_bool is_input, hi_u32 *total_line_num, hi_u32 *line_byte_num)
{
    ...
    *line_byte_num = (hi_u32)dims.dims[dims.dim_count - 1] *
    ...
    const sample_svp_npu_task_info *task)
{
    ...
    for (i = 0; i < file_num; i++) {
        ...
        for (line = 0; line < task->cfg.dynamic_batch_num * total_line_num; line++) {
            ret = (hi_s32)fread(data + line * stride, line_byte_num, 1, fp);
...
static hi_s32 sample_svp_npu_read_model(const hi_char *model_path, hi_u32 model_index, hi_bool is_cached)
{
    ...
    g_svp_npu_model[model_index].model_mem_size = (hi_ulong)ftell(fp);
    ...
    ret = (hi_s32)fread(g_svp_npu_model[model_index].model_mem_ptr, g_svp_npu_model[model_index].model_mem_size, 1, fp);
...
hi_s32 sample_common_svp_npu_create_task_buf(sample_svp_npu_task_info *task)
{
    ...
    sample_svp_check_exps_return(num != g_svp_npu_model[task->cfg.model_idx].input_num - SAMPLE_SVP_NPU_EXTRA_INPUT_NUM,
        HI_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "num of data buffer(%zu) should be %u when create task buf!\n",
...
hi_s32 sample_common_svp_npu_create_work_buf(sample_svp_npu_task_info *task)
{
    ...
    sample_svp_check_exps_return(num != g_svp_npu_model[task->cfg.model_idx].input_num - 1,
        HI_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "num of data buffer(%zu) should be %u when create work buf!\n",
...
hi_s32 sample_common_svp_npu_set_input_aipp_info(hi_u32 model_index, hi_u32 index,
    const sample_svp_npu_model_aipp *model_aipp, const sample_svp_npu_task_info *task)
{
    svp_acl_mdl_aipp *aipp_set = HI_NULL;
    ...
    hi_bool is_input)
{
    ...
    for (i = 0; i < data_num; i++) {
        ...
        for (line = 0; line < task->cfg.dynamic_batch_num * total_line_num; line++) {
            ret = (hi_s32)fwrite(data, line_byte_num, 1, fp);
            if (ret != 1) {
                fclose(fp);
...
hi_s32 sample_common_svp_npu_get_output_data_buffer_info(const sample_svp_npu_task_info *task, hi_u32 idx,
    hi_u8 **virt_addr, hi_u32 *size, hi_u32 *stride)
{
    svp_acl_data_buffer *data_buffer = HI_NULL;
...
static hi_bool sample_svp_npu_check_valid_rect(const hi_point *point)
{
    ...
    const hi_video_frame_info *proc_frame, const hi_video_frame_info *show_frame, hi_sample_svp_rect_info *rect_info)
{
    ...
    for (i = 0, j = 0; i < rect_info->num; i++) {
        ...
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].x = (hi_u32)((hi_float)x_min[roi_offset] /
            proc_frame->video_frame.width * show_frame->video_frame.width) & (~1);
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_LEFT_TOP].y = (hi_u32)((hi_float)y_min[roi_offset] /
            proc_frame->video_frame.height * show_frame->video_frame.height) & (~1);

        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_TOP].x = (hi_u32)((hi_float)x_max[roi_offset] /
            proc_frame->video_frame.width * show_frame->video_frame.width) & (~1);
        ...
        rect_info->rect[j].point[SAMPLE_SVP_NPU_RECT_RIGHT_BOTTOM].y = (hi_u32)((hi_float)y_max[roi_offset] /
            proc_frame->video_frame.height * show_frame->video_frame.height) & (~1);
        ...
        roi_offset++;
        if (sample_svp_npu_check_valid_rect(rect_info->rect[j].point) == HI_TRUE) {
            j++;
        }
    }
    rect_info->num = j;
    ...
    const hi_video_frame_info *show_frame, hi_sample_svp_coords_info *coords_info)
{
    ...
    c = (hi_u32)dim.dims[1];
    h = (hi_u32)dim.dims[SAMPLE_SVP_NPU_H_DIM_IDX];
    w = (hi_u32)dim.dims[SAMPLE_SVP_NPU_W_DIM_IDX];
    ...
    for (i = 0; i < c; i++) {
        ...
        if (px > 1 && px < (hi_s32)w - 1 && py > 1 && py < (hi_s32)h - 1) {
            double diffx = (double)(out[i * (h * stride) + (hi_u32)py * stride + px + 1] -
                out[i * (h * stride) + (hi_u32)py * stride + px - 1]);
            double diffy = (double)(out[i * (h * stride) + ((hi_u32)py + 1) * stride + px] -
                out[i * (h * stride) + (hi_u32)(py - 1) * stride + px]);
    ...
    hi_size *pic_size)
{
    ...
    sample_svp_check_exps_return(ret != SVP_ACL_SUCCESS, HI_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "get num dims failed!\n");
    pic_size->height = (hi_u32)dims.dims[dims.dim_count - SAMPLE_SVP_NPU_H_DIM_IDX]; // NCHW
    pic_size->width = (hi_u32)dims.dims[dims.dim_count - 1]; // NCHW
    return HI_SUCCESS;
}
