    ...
    hi_sample_svp_rect_info *rect, hi_u32 color)
{
    ...
    for (i = 0; i < rect->num; i++) {
        vgs_add_cover.quad.is_solid = HI_FALSE;
        vgs_add_cover.quad.thick = HI_SAMPLE_IVE_DRAW_THICK;
...
static hi_s32 sample_common_svp_start_vpss(hi_s32 vpss_grp_cnt, hi_size *pic_size, hi_u32 vpss_chn_num)
{
    ...
    vpss_grp_attr.nr_attr.compress_mode = HI_COMPRESS_MODE_NONE;
    ...
    for (i = 1; i < vpss_chn_num; i++) {
        (hi_void)memset_s(&vpss_chn_attr[i], sizeof(hi_vpss_chn_attr), 0, sizeof(hi_vpss_chn_attr));
    ...
    for (vpss_grp = 0; vpss_grp < vpss_grp_cnt; vpss_grp++) {
        ret = sample_common_vpss_start(vpss_grp, chn_enable, &vpss_grp_attr, vpss_chn_attr, HI_VPSS_MAX_CHN_NUM);
        if (ret != HI_SUCCESS) {
...
/* function : Stop Vpss */
static hi_void sample_common_svp_stop_vpss(hi_s32 vpss_grp_cnt, hi_u32 vpss_chn_num)
{
    hi_vpss_grp vpss_grp = 0;
    hi_bool chn_enable[HI_VPSS_MAX_CHN_NUM] = { HI_FALSE, HI_FALSE, HI_FALSE, HI_FALSE };
    hi_s32 i;
    for (i = 0; (i < vpss_chn_num) && (i < HI_VPSS_MAX_CHN_NUM); i++) {
        chn_enable[i] = HI_TRUE;
    }
...
static hi_s32 sample_common_svp_get_def_vo_cfg(sample_vo_cfg *vo_cfg)
{
    ...
    vo_cfg->vo_intf_type      = HI_VO_INTF_HDMI;
    vo_cfg->intf_sync         = HI_VO_OUT_1080P30;
 ...
 * function : Start Vo
 */
static hi_s32 sample_common_svp_start_vo(const hi_sample_svp_switch *vo_venc_switch, sample_vo_cfg *vo_cfg)
{
    hi_s32 ret;
    sample_svp_check_exps_return(vo_venc_switch == HI_NULL, HI_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
    ...
    if (vo_venc_switch->is_vo_open == HI_FALSE) {
        return HI_SUCCESS;
    }
 ...
 * function : Stop Vo
 */
static hi_void sample_common_svp_stop_vo(const hi_sample_svp_switch *vo_venc_switch, sample_vo_cfg *vo_cfg)
{
    sample_svp_check_exps_return_void(vo_venc_switch == HI_NULL, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "vo_venc_switch can't be null\n");
...
static hi_s32 sample_common_svp_start_venc(const hi_sample_svp_switch *switch_ptr,
    sample_vo_cfg *vo_cfg, hi_pic_size *pic_type, hi_u32 chn_num)
{
    ...
    hi_sample_svp_switch *switch_ptr, hi_pic_size *ext_pic_size_type)
{
    ...
    hi_pic_size venc_pic_type[SAMPLE_SVP_VDEC_CHN_NUM] = {PIC_1080P};
    ...
    return HI_SUCCESS;
end_init_4:
    sample_common_svp_stop_vpss(vpss_grp_cnt, HI_VPSS_CHN_NUM);
end_init_3:
 ...
 * Load bin
 */
hi_s32 sample_comm_svp_load_core_binary(hi_svp_dsp_id core_id)
{
    hi_s32 ret;
    hi_char *bin[HI_SVP_DSP_ID_BUTT][SAMPLE_SVP_DSP_BIN_NUM_PER] = {
    ...
    ret = hi_mpi_svp_dsp_power_on(core_id);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x):hi_mpi_svp_dsp_power_on failed!\n", ret);

    ret = hi_mpi_svp_dsp_load_bin(bin[core_id][SAMPLE_SVP_DSP_MEM_TYPE_IRAM],
        core_id * SAMPLE_SVP_DSP_BIN_NUM_PER + SAMPLE_SVP_DSP_MEM_TYPE_IRAM);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x): hi_mpi_svp_dsp_load_bin failed!\n", ret);

    ret = hi_mpi_svp_dsp_load_bin(bin[core_id][SAMPLE_SVP_DSP_MEM_TYPE_SYS_DDR],
        core_id * SAMPLE_SVP_DSP_BIN_NUM_PER + SAMPLE_SVP_DSP_MEM_TYPE_SYS_DDR);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x): hi_mpi_svp_dsp_load_bin failed!\n", ret);
    ret = hi_mpi_svp_dsp_load_bin(bin[core_id][SAMPLE_SVP_DSP_MEM_TYPE_DRAM_0],
        core_id * SAMPLE_SVP_DSP_BIN_NUM_PER + SAMPLE_SVP_DSP_MEM_TYPE_DRAM_0);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x): hi_mpi_svp_dsp_load_bin failed!\n", ret);
    ret = hi_mpi_svp_dsp_load_bin(bin[core_id][SAMPLE_SVP_DSP_MEM_TYPE_DRAM_1],
        core_id * SAMPLE_SVP_DSP_BIN_NUM_PER + SAMPLE_SVP_DSP_MEM_TYPE_DRAM_1);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x): hi_mpi_svp_dsp_load_bin failed!\n", ret);

    ret = hi_mpi_svp_dsp_enable_core(core_id);
    sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x): hi_mpi_svp_dsp_enable_core failed!\n", ret);
    return ret;
}
 ...
 * UnLoad bin
 */
void sample_comm_svp_unload_core_binary(hi_svp_dsp_id core_id)
{
    hi_s32 ret;
    ret = hi_mpi_svp_dsp_disable_core(core_id);
    sample_svp_check_exps_return_void(ret != HI_SUCCESS, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x):hi_mpi_svp_dsp_disable_core failed!\n", ret);

    ret = hi_mpi_svp_dsp_power_off(core_id);
    sample_svp_check_exps_return_void(ret != HI_SUCCESS, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x):hi_mpi_svp_dsp_power_off failed!\n", ret);
}
...
static hi_s32 sample_common_svp_vpss_attach_vb(hi_sample_svp_media_cfg *media_cfg, hi_s32 vpss_grp_cnt)
{
    ...
    for (grp = 0; grp < vpss_grp_cnt; grp++) {
        for (chn = 0; chn < media_cfg->chn_num; chn++) {
            ret = hi_mpi_vpss_attach_vb_pool(grp, chn, media_cfg->vb_pool[chn]);
            if (ret == HI_SUCCESS) {
                continue;
            }
            for (chn_tmp = 0; chn_tmp < chn; chn_tmp++) {
                (hi_void)hi_mpi_vpss_detach_vb_pool(grp, chn_tmp);
            }
            sample_svp_check_exps_goto(ret != HI_SUCCESS, end_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
                "Error(%#x), hi_mpi_vpss_attach_vb_pool failed!\n", ret);
        }
    }
    return ret;
end_0:
    for (grp_tmp = 0; grp_tmp < grp; grp_tmp++) {
        for (chn_tmp = 0; chn_tmp < media_cfg->chn_num; chn_tmp++) {
            (hi_void)hi_mpi_vpss_detach_vb_pool(grp_tmp, chn_tmp);
        }
    }
    return ret;
}
...
static hi_void sample_common_svp_vpss_detach_vb(hi_sample_svp_media_cfg *media_cfg, hi_s32 vpss_grp_cnt)
{
    ...
    for (grp = 0; grp < vpss_grp_cnt; grp++) {
        for (chn = 0; chn < media_cfg->chn_num; chn++) {
            (hi_void)hi_mpi_vpss_detach_vb_pool(grp, chn);
        }
    }
}
