    ...
    if (u32ISO != u32LastISO)
    {
        ...
        if(stVPSSNRXParam.stNRXParam_V2.stNRXManual.stNRXParam.SFy[1].kMode > 1)
        {
            for(j = 0; j < 32; j++)
            {
                ...
                //stVPSSNRXParam.stNRXParam_V2.stNRXManual.stNRXParam.SBSk2[j] = g_astScenePipeParam[u8Index].stDynamicThreeDNR.astThreeDNRVPSSValue[u32IsoLevel].SBSk2[j];
                //stVPSSNRXParam.stNRXParam_V2.stNRXManual.stNRXParam.SDSk2[j] = g_astScenePipeParam[u8Index].stDynamicThreeDNR.astThreeDNRVPSSValue[u32IsoLevel].SDSk2[j];
            }
        }
        if(stVPSSNRXParam.stNRXParam_V2.stNRXManual.stNRXParam.SFy[2].kMode > 1)
        {
            for(j = 0; j < 32; j++)
            {
                ...
                //stVPSSNRXParam.stNRXParam_V2.stNRXManual.stNRXParam.SBSk3[j] = g_astScenePipeParam[u8Index].stDynamicThreeDNR.astThreeDNRVPSSValue[u32IsoLevel].SBSk3[j];
                //stVPSSNRXParam.stNRXParam_V2.stNRXManual.stNRXParam.SDSk3[j] = g_astScenePipeParam[u8Index].stDynamicThreeDNR.astThreeDNRVPSSValue[u32IsoLevel].SDSk3[j];
            }
        }
...
HI_S32 HI_SCENE_SetDynamicFsWdr_AutoGenerate(VI_PIPE ViPipe, HI_U32 u32ISO, HI_U32 u32LastISO, HI_U8 u8Index, HI_U32 u32ActRation)
{
    ...
    if (u32ISO != u32LastISO)
    {
        ...
        if (u32ISO < 250)
        {
            stBayerNRAttr.stWdr.au8FusionFrameStr[0] = 15;
            stBayerNRAttr.stWdr.au8FusionFrameStr[1] = 14;
        }
        else if ((u32ISO >= 250) && (u32ISO <= 350))
        {
            stBayerNRAttr.stWdr.au8FusionFrameStr[0] = 20;
            stBayerNRAttr.stWdr.au8FusionFrameStr[1] = 15;
        }
        else
        {
            stBayerNRAttr.stWdr.au8FusionFrameStr[0] = 26;
            stBayerNRAttr.stWdr.au8FusionFrameStr[1] = 16;
        }
        CHECK_SCENE_PAUSE();
        HI_MPI_ISP_SetNRAttr(ViPipe, &stBayerNRAttr);
        CHECK_SCENE_RET(s32Ret);
...
HI_S32 HI_SCENE_GetModuleState_AutoGenerate(HI_SCENE_MODULE_STATE_S *pstModuleState)
{
    ...
    pstModuleState->bStaticSaturation = 1;
    pstModuleState->bStaticCCM = 0;
    pstModuleState->bStaticShading = 1;
    pstModuleState->bStaticCSC = 0;
    pstModuleState->bStaticStatistics = 1;
    pstModuleState->bStaticDemosaic = 1;
    pstModuleState->bStaticSharpen = 1;
    pstModuleState->bStaticDetail = 1;
    pstModuleState->bDynamicBayernr = 1;
    pstModuleState->bDynamicThreeDNR = 1;
    pstModuleState->bDynamicGamma = 1;
    pstModuleState->bDynamicAE = 1;
    pstModuleState->bDynamicFalseColor = 0;
