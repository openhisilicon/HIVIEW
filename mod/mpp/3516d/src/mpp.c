	   ...
	   // start vdec;
		{
            ...
            /*SAMPLE_VDEC_ATTR astSampleVdec;
			...
			s32Ret = SAMPLE_COMM_VDEC_Start(1, &astSampleVdec);
			if (s32Ret != HI_SUCCESS)
			{
				SAMPLE_PRT("start VDEC fail for %#x!\n", s32Ret);
				return;
			}
			printf("start vdec ok...\n");*/
			...
			stChnAttr.u32FrameBufCnt = 2 /*u32RefFrameNum*/ + 2 /*u32DisplayFrameNum*/ + 1;
			...
			if (PT_H264 == stChnAttr.enType || PT_H265 == stChnAttr.enType)
			{
				stChnParam.stVdecVideoParam.enDecMode = VIDEO_DEC_MODE_IP;
		...
		{
			VPSS_GRP_ATTR_S stGrpAttr = { 0 };
			VPSS_CHN_ATTR_S stChnAttr[VPSS_MAX_CHN_NUM] = { 0 };

			/*** create vpss group ***/
            stGrpAttr.u32MaxW = 2688;// ALIGN_UP(attr->width, 16);
            stGrpAttr.u32MaxH = 2160;// ALIGN_UP(attr->height, 16);
			stGrpAttr.stFrameRate.s32SrcFrameRate = -1;
			stGrpAttr.stFrameRate.s32DstFrameRate = -1;
			stGrpAttr.enDynamicRange = DYNAMIC_RANGE_SDR8;
			stGrpAttr.enPixelFormat = PIXEL_FORMAT_YVU_SEMIPLANAR_420;
			...
			for (i = 0; i < 2; i++)
			{
                stChnAttr[i].u32Width = 1920;// ALIGN_UP((i == 0) ? attr->width : 640, 16);
                stChnAttr[i].u32Height = 1080;//ALIGN_UP((i == 0) ? attr->height : 480, 16);
				...
				/* HI_BOOL chen[VPSS_MAX_PHY_CHN_NUM] = { HI_TRUE,HI_TRUE };
				 s32Ret = SAMPLE_COMM_VPSS_Start(i, chen, &stGrpAttr, &stChnAttr[i]);
				 CHECK_CHN_RET(s32Ret, i, "SAMPLE_COMM_VPSS_Start");*/
            }
