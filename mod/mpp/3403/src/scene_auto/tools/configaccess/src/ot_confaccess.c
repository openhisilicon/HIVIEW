...
static hi_s32 confaccess_get_all_modules_info(hi_s32 module_num, conf_access_node *cfg_pos, const hi_char *cfg_path_buf)
{
    ...
    for (hi_s32 index = 0; index < module_num; ++index) {
        ...
        const hi_char *value = iniparser_getstring(cfg_pos->module.ini_dir, (const char *)module_node_key, NULL);
        if (value == NULL) {
        ...
        snprintf_truncated_s(module_info->ini_file, HI_CONFACCESS_PATH_MAX_LEN, "%s%c%s", module_info->path,
...
static hi_s32 confaccess_init(const hi_char *cfg_name, const hi_char *cfg_path, hi_u32 *out_module_num)
{
    ...
    *out_module_num = (hi_u32)iniparser_getint(cfg_pos->module.ini_dir, (hi_char *)"module:module_num", 0);
#ifndef __LITEOS__
        ...
        if (strncmp(cfg_node->cfg_name, cfg_name, HI_CONFACCESS_NAME_MAX_LEN) == 0) {
            ...
            ot_list_for_each(module_pos, &(cfg_node->module.list)) {
                ...
                if (strncmp(cfg_module->name, module, HI_CONFACCESS_NAME_MAX_LEN) == 0) {
                    *value_vector = (hi_char *)iniparser_getstring(cfg_module->ini_dir, conf_item, default_val);
