...
hi_void mcf_pipe_change(hi_vi_pipe *vi_pipe, hi_u8 index)
{
    if (g_scene_mode.pipe_attr[index].pipe_type == HI_SCENE_PIPE_TYPE_MCF_3DNR) {
        *vi_pipe = 1;
    } else {
    }
}
        ...
        for (hi_u32 j = 0; j < g_scene_state.main_pipe[i].sub_pipe_num; j++) {
            ...
            if (g_scene_mode.pipe_attr[index].pipe_type == HI_SCENE_PIPE_TYPE_SNAP) {
                ...
                ot_scenecomm_check(ret, HI_SCENE_EINTER);
                       ...
                       g_scene_mode.pipe_attr[index].pipe_type == HI_SCENE_PIPE_TYPE_MCF_3DNR) {
...
static hi_s32 scene_set_pipe_wdr_dynamic_param_part1(hi_void *arg)
{
    hi_s32 ret;
    hi_u32 i;
...
hi_void *scene_not_linear_auto_thread(hi_void *arg)
{
    ...
    while (g_scene_state.thread_not_linear.thread_flag == HI_TRUE) {
        ...
        for (i = 0; i < g_scene_state.main_pipe_num; i++) {
            hi_vi_pipe vi_pipe = g_scene_state.main_pipe[i].main_pipe_hdl;
            ot_scenecomm_expr_true_return(vi_pipe >= HI_SCENE_PIPE_MAX_NUM, NULL);
        }
...
{
    for (hi_u32 i = 0; i < HI_SCENE_PIPE_MAX_NUM; i++) {
