...
static hi_void sample_audio_exit_proc(hi_void)
{
    ...
    for (dev_id = 0; dev_id < HI_AI_DEV_MAX_NUM; dev_id++) {
        for (chn_id = 0; chn_id < HI_AI_MAX_CHN_NUM; chn_id++) {
            if (sample_comm_audio_destroy_trd_ai_ext_res(dev_id, chn_id) != HI_SUCCESS) {
...
static hi_void ai_init_talk_vqe_v2_param(hi_ai_talk_vqe_v2_cfg *ai_vqe_talk_v2_attr)
{
    ...
    ai_vqe_talk_v2_attr->open_mask = HI_AI_TALKVQEV2_MASK_AGC | HI_AI_TALKVQEV2_MASK_PNR | HI_AI_TALKVQEV2_MASK_WNR;
}
    ...
    hi_aio_attr *hdmi_ao_attr)
{
    ...
    *ai_dev = SAMPLE_AUDIO_EXTERN_AI_DEV;
    ...
    hdmi_ao_attr->snd_mode      = HI_AUDIO_SOUND_MODE_STEREO;
...
hi_s32 sample_audio_register_vqe_module(hi_void)
{
    ...
    vqe_reg_cfg.wnr_mod_cfg.handle = hi_vqe_wnr_get_handle();
