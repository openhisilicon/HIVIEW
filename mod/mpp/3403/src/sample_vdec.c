...
static hi_s32 sample_vdec_unbind_vpss(hi_u32 vpss_grp_num)
{
    hi_u32 i;
    hi_s32 ret = HI_SUCCESS;
    for (i = 0; i < vpss_grp_num; i++) {
...
static hi_s32 sample_config_vpss_ldy_attr(hi_u32 vpss_grp_num)
{
    ...
    for (i = 0; i < vpss_grp_num; i++) {
        ret = hi_mpi_vpss_get_low_delay_attr(i, 0, &vpss_ldy_info);
        if (ret != HI_SUCCESS) {
        ...
        ret = hi_mpi_vpss_set_low_delay_attr(i, 0, &vpss_ldy_info);
        if (ret != HI_SUCCESS) {
...
static hi_s32 sample_start_vpss(hi_vpss_grp *vpss_grp, hi_u32 vpss_grp_num, hi_bool *vpss_chn_enable, hi_u32 arr_len)
{
    ...
    vpss_chn_attr[0].compress_mode = HI_COMPRESS_MODE_SEG;
    ...
    vpss_chn_attr[0].border_en                 = HI_FALSE;
...
static hi_s32 sample_vpss_bind_vo(sample_vo_cfg vo_config, hi_u32 vpss_grp_num)
{
    hi_u32 i;
...
hi_s32 sample_start_vo(sample_vo_cfg *vo_config, hi_u32 vpss_grp_num)
{
    hi_s32 ret;
