...
#define MIPI_LANE_NUM_PER_DEV 4
...
typedef enum {
    MIPI_TX_CLK_LANE_CONTINUE     = 0x0,
    MIPI_TX_CLK_LANE_NON_CONTINUE = 0x1,
    MIPI_TX_CLK_LANE_BUTT
} continue_mode_t;
...
typedef struct {
    ...
    continue_mode_t     clklane_continue_mode;
} combo_dev_cfg_t;
...
typedef struct {
    unsigned int devno;
    unsigned char pn_swap[MIPI_LANE_NUM_PER_DEV];
} combo_dev_pn_swap_t;
...
#define OT_MIPI_TX_SET_PN_SWAP      _IOW(OT_MIPI_TX_IOC_MAGIC, 0x06, combo_dev_pn_swap_t)

#endif /* end of #ifndef OT_MIPI_TX_H */
