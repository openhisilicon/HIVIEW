...
hi_s32 sample_comm_audio_destory_all_thread(void)
{
    hi_u32 dev_id, chn_id;

    for (dev_id = 0; dev_id < HI_AI_DEV_MAX_NUM; dev_id++) {
        for (chn_id = 0; chn_id < HI_AI_MAX_CHN_NUM; chn_id++) {
            if (sample_comm_audio_destory_thread_ai(dev_id, chn_id) != HI_SUCCESS) {
                printf("%s: sample_comm_audio_destory_thread_ai(%d,%d) failed!\n", __FUNCTION__, dev_id, chn_id);
                return HI_FAILURE;
            }
        }
    }
    ...
    for (chn_id = 0; chn_id < HI_AENC_MAX_CHN_NUM; chn_id++) {
        if (sample_comm_audio_destory_thread_aenc_adec(chn_id) != HI_SUCCESS) {
            printf("%s: sample_comm_audio_destory_thread_aenc_adec(%d) failed!\n", __FUNCTION__, chn_id);
            return HI_FAILURE;
        }
    }

    for (chn_id = 0; chn_id < HI_ADEC_MAX_CHN_NUM; chn_id++) {
        if (sample_comm_audio_destory_thread_file_adec(chn_id) != HI_SUCCESS) {
            printf("%s: sample_comm_audio_destory_thread_file_adec(%d) failed!\n", __FUNCTION__, chn_id);
            return HI_FAILURE;
        }
    }

    for (chn_id = 0; chn_id < HI_AO_DEV_MAX_NUM; chn_id++) {
        if (sample_comm_audio_destory_thread_ao_vol_ctrl(chn_id) != HI_SUCCESS) {
            printf("%s: sample_comm_audio_destory_thread_ao_vol_ctrl(%d) failed!\n", __FUNCTION__, chn_id);
            return HI_FAILURE;
        }
    }
...
hi_s32 sample_comm_audio_start_adec(hi_u32 adec_chn_cnt, const hi_aio_attr *aio_attr, hi_payload_type type)
{
    ...
    if (adec_attr.type == HI_PT_ADPCMA) {
        ...
        adec_attr.mode = HI_ADEC_MODE_STREAM;
