...
hi_s32 sample_inner_codec_cfg_audio(hi_audio_sample_rate sample_rate)
{
    ...
    input_mode = HI_ACODEC_MIXER_IN_D;  // pseudo-differential (IN_D)
    //input_mode = HI_ACODEC_MIXER_IN1;   // single-ended (IN1)
    ...
    #if 0
    acodec_input_vol = 10;  //[0, 15]，其中0到15按2db递增，0对应0db，15对应最大增益30db
    ret = ioctl(fd_acodec,  HI_ACODEC_GET_GAIN_MICL, &acodec_input_vol);
    if (ret != HI_SUCCESS) {
        printf("%s: set acodec micin gain failed\n", __FUNCTION__, acodec_input_vol);
        goto cfg_fail;
    }
    printf("%s: get acodec micin gain MICL:%d\n", __FUNCTION__, acodec_input_vol);
    
    acodec_input_vol = 10;
    ret = ioctl(fd_acodec,  HI_ACODEC_GET_GAIN_MICR, &acodec_input_vol);
    if (ret != HI_SUCCESS) {
        printf("%s: get acodec micin gain failed\n", __FUNCTION__);
        goto cfg_fail;
    }
    printf("%s: set acodec micin gain MICR:%d\n", __FUNCTION__, acodec_input_vol);
    #endif
