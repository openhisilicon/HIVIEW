...
hi_s32 sample_comm_audio_creat_thread_ai_ext_res(hi_aio_attr *aio_attr, hi_audio_dev ai_dev,
...
static hi_void sample_audio_exit_proc(hi_void)
{
    ...
    sample_comm_audio_destory_all_thread();
...
hi_void sample_audio_adec_ao_inner(hi_audio_dev ao_dev, hi_ao_chn ao_chn, hi_adec_chn ad_chn)
{
    ...
    ret = sample_comm_audio_creat_thread_file_adec(ad_chn, fd);
    if (ret != HI_SUCCESS) {
        sample_dbg(ret);
        fclose(fd);
        fd = HI_NULL;
    ...
    ret = sample_comm_audio_destory_thread_file_adec(ad_chn);
    if (ret != HI_SUCCESS) {
        sample_dbg(ret);
    }
...
static hi_s32 sample_audio_aenc_bind_ai(hi_audio_dev ai_dev, hi_u32 aenc_chn_cnt)
{
    ...
    for (i = 0; i < aenc_chn_cnt; i++) {
        ...
        if (g_user_get_mode == HI_TRUE) {
            ret = sample_comm_audio_creat_thread_ai_aenc(ai_dev, ai_chn, ae_chn);
        } else {
...
aenc_bind_err:
    for (j = 0; j < i; j++) {
        ...
        if (g_user_get_mode == HI_TRUE) {
            sample_comm_audio_destory_thread_ai(ai_dev, ai_chn);
        } else {
...
static hi_void sample_audio_aenc_unbind_ai(hi_audio_dev ai_dev, hi_u32 aenc_chn_cnt)
{
    ...
    for (i = 0; i < aenc_chn_cnt; i++) {
        ...
        if (g_user_get_mode == HI_TRUE) {
            ret = sample_comm_audio_destory_thread_ai(ai_dev, ai_chn);
            if (ret != HI_SUCCESS) {
                sample_dbg(ret);
            }
        } else {
...
static hi_s32 sample_audio_adec_bind_aenc(hi_u32 adec_chn_cnt)
{
    ...
    for (i = 0; i < adec_chn_cnt; i++) {
        ...
        if (fd == HI_NULL) {
            sample_dbg(HI_FAILURE);
            for (j = 0; j < i; j++) {
                sample_comm_audio_destory_thread_aenc_adec(j);
            }
            return HI_FAILURE;
        }

        ret = sample_comm_audio_creat_thread_aenc_adec(ae_chn, ad_chn, fd);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
            fclose(fd);
            fd = HI_NULL;
...
static hi_void sample_audio_adec_unbind_aenc(hi_u32 adec_chn_cnt)
{
    ...
    for (i = 0; i < adec_chn_cnt; i++) {
        ad_chn = i;
        ret = sample_comm_audio_destory_thread_aenc_adec(ad_chn);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
        }
    }
...
hi_s32 sample_audio_ai_aenc(gsf_mpp_aenc_t *aenc)
{
    ...
    #if 1 //for single-ended MIC;
    //IN1R => IN1L_R
    hi_audio_track_mode track_mode = HI_AUDIO_TRACK_BOTH_RIGHT; // HI_AUDIO_TRACK_EXCHANGE;
    ...
    for (i = 0; i < adec_chn_cnt; i++) {
        ...
        ret = sample_comm_audio_creat_thread_aenc_adec(ae_chn, ad_chn, NULL);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
            for (j = 0; j < i; j++) {
                sample_comm_audio_destory_thread_aenc_adec(j);
...
hi_s32 sample_audio_ai_aenc_stop(gsf_mpp_aenc_t *aenc)
{
    ...
    //sample_comm_audio_destory_thread_aenc_adec
    sample_audio_adec_unbind_aenc(adec_chn_cnt);
...
static hi_s32 sample_audio_start_ai_resample(hi_u32 ai_chn_cnt, hi_audio_dev ai_dev, hi_aio_attr *aio_attr)
{
    ...
    for (i = 0; i < ai_chn_cnt; i++) {
        ...
        ret = sample_comm_audio_creat_thread_ai_ext_res(aio_attr, ai_dev, ai_chn, g_out_sample_rate, fd);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
            fclose(fd);
            fd = HI_NULL;
    ...
    hi_audio_dev ao_dev, hi_ao_chn ao_chn)
{
    hi_s32 ret;
    if (g_user_get_mode == HI_TRUE) {
        ret = sample_comm_audio_creat_thread_ai_ao(ai_dev, ai_chn, ao_dev, ao_chn);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
            return HI_FAILURE;
        }
    ...
    hi_audio_dev ao_dev, hi_ao_chn ao_chn)
{
    ...
    if (g_ao_volume_ctrl == HI_TRUE) {
        ret = sample_comm_audio_creat_thread_ao_vol_ctrl(ao_dev);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
            goto ai_ao_err0;
        }
...
#endif
    if (g_ao_volume_ctrl == HI_TRUE) {
        ret = sample_comm_audio_destory_thread_ao_vol_ctrl(ao_dev);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
        }
    }
    ...
    hi_audio_dev ao_dev)
{
    ...
    /* bind AI to AO channel */
    ret = sample_comm_audio_creat_thread_ai_ao(ai_dev, ai_chn, ao_dev, HI_AO_SYS_CHN_ID);
    if (ret != HI_SUCCESS) {
        sample_dbg(ret);
        return;
    }
    ...
    hi_audio_dev ao_dev)
{
    ...
    for (i = 0; i < (ai_chn_cnt >> ((hi_u32)ai_attr->snd_mode)); i++) {
        ret = sample_comm_audio_creat_thread_ai_ao(ai_dev, i, ao_dev, i);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
            for (j = 0; j < i; j++) {
                sample_comm_audio_destory_thread_ai(ai_dev, j);
            }
            return HI_FAILURE;
        }
...
static hi_void sample_audio_ao_unbind_ai_multi_chn(hi_audio_dev ai_dev, const hi_aio_attr *ai_attr)
{
    ...
    for (i = 0; i < (ai_chn_cnt >> ((hi_u32)ai_attr->snd_mode)); i++) {
        ret = sample_comm_audio_destory_thread_ai(ai_dev, i);
        if (ret != HI_SUCCESS) {
            sample_dbg(ret);
        }
    }
...
static hi_void sample_audio_dimc_ai_ao_init_param(hi_aio_attr *ai_attr, hi_audio_dev *ai_dev,
    hi_aio_attr *ao_attr, hi_audio_dev *ao_dev)
{
    ai_attr->sample_rate  = HI_AUDIO_SAMPLE_RATE_48000;
    ai_attr->bit_width    = HI_AUDIO_BIT_WIDTH_16;
    ai_attr->work_mode    = HI_AIO_MODE_I2S_MASTER;
    ai_attr->snd_mode     = HI_AUDIO_SOUND_MODE_STEREO;
    ...
    ao_attr->sample_rate  = HI_AUDIO_SAMPLE_RATE_48000;
    ao_attr->bit_width    = HI_AUDIO_BIT_WIDTH_16;
    ao_attr->work_mode    = HI_AIO_MODE_I2S_MASTER;
    ao_attr->snd_mode     = HI_AUDIO_SOUND_MODE_STEREO;
    ao_attr->expand_flag  = 0;
    ao_attr->frame_num    = SAMPLE_AUDIO_AI_USER_FRAME_DEPTH;
    ao_attr->point_num_per_frame = HI_AACLC_SAMPLES_PER_FRAME;
    ...
    *ao_dev = SAMPLE_AUDIO_INNER_AO_DEV;
    ai_attr->clk_share  = 0;
    ao_attr->clk_share  = 0;
    ai_attr->i2s_type   = HI_AIO_I2STYPE_DMIC;
#ifdef OT_ACODEC_TYPE_INNER
    ao_attr->i2s_type   = HI_AIO_I2STYPE_INNERCODEC;
#else
    ao_attr->i2s_type   = HI_AIO_I2STYPE_EXTERN;
#endif
}

static hi_void sample_audio_dmic_ai_ao_inner(hi_audio_dev ai_dev, hi_ai_chn ai_chn,
    hi_audio_dev ao_dev, hi_ao_chn ao_chn)
{
    hi_s32 ret;
...
/* function : ai(dmic) -> ao */
hi_s32 sample_audio_dmic_ai_ao(hi_void)
{
    hi_s32 ret;
    hi_audio_dev ai_dev, ao_dev;
    hi_aio_attr ai_attr, ao_attr;
    hi_u32 ai_chn_cnt, ao_chn_cnt;
    sample_comm_ai_vqe_param ai_vqe_param = {0};
    const hi_ai_chn ai_chn = 0;
    const hi_ao_chn ao_chn = 0;
    ...
    ret = hi_mpi_ai_set_dmic_gain(ai_dev, SAMPLE_DMIC_GAIN);
    if (ret != HI_SUCCESS) {
        sample_dbg(ret);
        goto dmic_err2;
    }
