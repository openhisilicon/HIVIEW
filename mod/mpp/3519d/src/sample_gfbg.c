...
#ifdef CONFIG_SUPPORT_SAMPLE_ROTATION
static hi_s32 gfbg_put_layer_info(pthread_gfbg_sample_info *info);
static hi_s32 gfbg_get_canvas(pthread_gfbg_sample_info *info);
static hi_void gfbg_put_canvas(hi_void);
static hi_void gfbg_init_surface(hi_tde_surface *, hi_tde_surface *, hi_tde_rect *, hi_tde_rect *, hi_u32);
static hi_s32 gfbg_draw(pthread_gfbg_sample_info *info);
static hi_s32 gfbg_refresh(const pthread_gfbg_sample_info *info);
static hi_void gfbg_get_var_by_format(pthread_gfbg_sample_info *info, struct fb_var_screeninfo *var_info);
static hi_void gfbg_put_rotation_degree(struct fb_var_screeninfo *var_info, hi_fb_rotate_mode *rotate_mode);
static hi_s32 gfbg_put_rotation(pthread_gfbg_sample_info *info);
static hi_void gfbg_rotate(hi_vo_dev vo_dev);
#endif
static hi_void sample_gfbg_to_exit_signal(hi_void);

static int sample_gfbg_getchar(hi_void)
{
    int c;
...
static hi_s32 sample_gfbg_load_bmp(const char *filename, hi_u8 *addr)
{
    osd_surface surface;
...
static hi_void sample_gfbg_to_exit(hi_void)
{
    hi_char ch;
    while (1) {
...
static hi_s32 sample_get_file_name(pthread_gfbg_sample_info *info, hi_char *file, hi_u32 file_length)
{
    switch (info->layer) {
...
static hi_s32 sample_init_frame_buffer(pthread_gfbg_sample_info *info, const char *input_file)
{
    hi_bool show;
...
static hi_s32 sample_init_var(pthread_gfbg_sample_info *info)
{
...
static hi_s32 sample_put_alpha_and_colorkey(pthread_gfbg_sample_info *info, hi_fb_color_format clr_fmt)
{
    hi_s32 ret;
    hi_fb_alpha alpha = {0};
    ...
    color_key.value = (clr_fmt == HI_FB_FORMAT_ARGB8888) ? GFBG_RED_8888 : GFBG_RED_1555;
    ...
    for (y = 100; y < 300; y++) { /* 100 300 for y */
        for (x = 0; x < 300; x++) { /* 300 for x */
            if (clr_fmt == HI_FB_FORMAT_ARGB8888) {
                *((hi_u32*)ptemp + y * var->xres + x) = GFBG_RED_8888;
            } else {
                *((hi_u16*)ptemp + y * var->xres + x) = GFBG_RED_1555;
            }
        }
    }
    return;
...
static hi_s32 sample_time_to_play(pthread_gfbg_sample_info *info, hi_u8 *show_screen, hi_u32 fix_screen_stride)
{
    hi_bool show;
    ...
    hi_u32 fix_screen_stride, hi_u64 hide_screen_phy)
{
    ...
    dst_surface.phys_addr = (hi_phys_addr_t)hide_screen_phy;
...
static hi_void sample_get_param(pthread_gfbg_sample_info *info, hi_fb_color_format *clr_fmt,
...
static hi_s32 sample_draw_line_by_cpu(pthread_gfbg_sample_info *info, hi_u8 *show_screen, hi_u32 fix_screen_stride,
    hi_u64 *hide_screen_phy, hi_u32 i)
{
    hi_s32 x, y;
...
static hi_s32 sample_draw_line_and_picture(pthread_gfbg_sample_info *info, hi_void *viraddr, hi_u8 *show_screen,
    hi_u32 fix_screen_stride)
{
    ...
    for (i = 0; i < SAMPLE_IMAGE_NUM; i++) {
        ...
        dst = (hi_u8*)viraddr;
        if (sample_gfbg_load_bmp(image_name, dst) != HI_SUCCESS) {
            return HI_FAILURE;
        }
...
static hi_s32 sample_move_cursor(pthread_gfbg_sample_info *info, struct fb_var_screeninfo *var, hi_u8 *show_screen)
{
    hi_s32 i;
    hi_fb_point point = {0, 0};
    ...
    if (sample_gfbg_load_bmp(SAMPLE_CURSOR_PATH, show_screen) != HI_SUCCESS) {
        return HI_FAILURE;
    }
    if (ioctl(info->fd, FBIOPAN_DISPLAY, var) < 0) {
        sample_print("FBIOPAN_DISPLAY failed!\n");
...
static hi_s32 sample_show_bitmap(pthread_gfbg_sample_info *info, hi_u8 *show_screen, hi_u32 fix_screen_stride,
    hi_u32 byte_per_pixel)
{
    hi_s32 ret;
    struct fb_var_screeninfo var;
...
static hi_s32 sample_show_process(pthread_gfbg_sample_info *info)
{
...
static hi_void* sample_gfbg_pandisplay(hi_void *data)
{
    pthread_gfbg_sample_info *info = HI_NULL;
...
static hi_s32 sample_init_frame_buffer_ex(pthread_gfbg_sample_info *info, const char *input_file)
{
    hi_fb_colorkey color_key;
    hi_fb_point point = {0, 0};
    hi_char file[PATH_MAX + 1] = {0};
    ...
    if (ioctl(info->fd, FBIOPUT_COLORKEY_HIFB, &color_key) < 0) {
        sample_print("FBIOPUT_COLORKEY_HIFB!\n");
        close(info->fd);
        info->fd = -1;
        return HI_FAILURE;
    }
...
static hi_s32 sample_init_var_ex(pthread_gfbg_sample_info *info)
{
    struct fb_var_screeninfo var;
...
static hi_s32 sample_init_layer_info(pthread_gfbg_sample_info *info)
{
...
static hi_s32 sample_init_compress(pthread_gfbg_sample_info *info)
{
    ...
    if (info->compress == HI_TRUE) {
        if (ioctl(info->fd, FBIOPUT_COMPRESSION_HIFB, &info->compress) < 0) {
            sample_print("FBIOPUT_COMPRESSION_HIFB failed!\n");
            close(info->fd);
            info->fd = -1;
            return HI_FAILURE;
        }
...
static hi_s32 sample_init_canvas(pthread_gfbg_sample_info *info, hi_fb_buf *canvas_buf, hi_void **buf,
    hi_void **viraddr)
{
    hi_u32 byte_per_pixel;
    hi_fb_color_format clr_fmt;
...
static hi_s32 sample_draw_line_by_cpu_ex(pthread_gfbg_sample_info *info, hi_fb_buf *canvas_buf, hi_void *buf)
{
    hi_u32 x, y, color;
    hi_s32 ret;
    hi_fb_color_format clr_fmt;
...
static hi_void sample_source_cfg_tde(pthread_gfbg_sample_info *info, hi_tde_surface *src_surface, hi_tde_rect *src_rect,
...
static hi_s32 sample_draw_picture_by_tde(pthread_gfbg_sample_info *info, hi_fb_buf *canvas_buf, hi_u32 i,
    hi_void *viraddr)
{
    hi_char image_name[128]; /* 128 for char length */
    hi_u8 *dst = HI_NULL;
    ...
    /* 3. submit job */
    ret = hi_tde_end_job(handle, HI_FALSE, HI_TRUE, 10); /* timeout 10ms */
    if (ret < 0) {
...
static hi_s32 sample_time_to_play_ex(pthread_gfbg_sample_info *info, hi_fb_buf *canvas_buf, hi_void *buf,
    hi_void *viraddr)
{
    hi_s32 ret;
    hi_u32 i;
...
static hi_void* sample_gfbg_refresh(hi_void *data)
{
    hi_char file[FILE_LENGTH_MAX] = {0};
...
static hi_s32 sample_var_init(pthread_gfbg_sample_info *info, struct fb_var_screeninfo *var)
{
    if (info == HI_NULL) {
        return HI_FAILURE;
    }
...
static hi_s32 sample_cmap_init(pthread_gfbg_sample_info *info)
{
    struct fb_cmap cmap;
    cmap.start = 0;
...
static hi_s32 sample_get_fix_and_mmap(pthread_gfbg_sample_info *info, struct fb_fix_screeninfo *fix,
                                      hi_void **viraddr)
{
...
static hi_s32 sample_start_draw_rect(hi_tde_none_src *none_src, hi_tde_corner_rect_info *conner_rect)
{
    ...
    for (i = 0; i < 5; i++) { /* 5 draw 5 rect */
        if (hi_tde_draw_corner_box(handle, none_src->dst_surface, &corner, 1) < 0) {
            hi_tde_cancel_job(handle);
            return HI_FAILURE;
        }
...
static hi_s32 sample_draw_rect(hi_void *viraddr, hi_u32 index,
...
static hi_s32 sample_pandisplay(pthread_gfbg_sample_info *info, hi_u32 index,
                                struct fb_var_screeninfo *var)
{
...
static hi_void *sample_gfbg_g0_clut(void *data)
{
    pthread_gfbg_sample_info *info = HI_NULL;
    struct fb_var_screeninfo var;
    struct fb_fix_screeninfo fix;
...
static hi_void sample_gfbg_handle_sig(hi_s32 signo)
{
...
static hi_void sample_gfbg_usage2(hi_void)
{
    printf("\n\n/****************index******************/\n");
...
static hi_void sample_gfbg_usage1(hi_char *s_prg_nm)
{
...
static hi_s32 sample_gfbg_start_vo(vo_device_info *vo_dev_info)
{
#if GFBG_BE_WITH_VO
    hi_vo_intf_type vo_intf_type = vo_dev_info->hi_vo_intf_type;
    ...
    if (vo_intf_type & HI_VO_INTF_BT1120) {
...
static hi_void sample_gfbg_stop_vo(vo_device_info *vo_dev_info)
{
#if GFBG_BE_WITH_VO
    hi_vo_dev vo_dev = vo_dev_info->vo_dev;
...
static hi_u32 sample_gfbg_vo_dev_get_layer(hi_vo_dev vo_dev)
{
...
static hi_s32 sample_gfbg_standard_mode(vo_device_info *vo_dev_info)
{
    hi_s32 ret;
    ...
    if (pthread_create(&g_gfbg_thread, 0, sample_gfbg_pandisplay, (hi_void *)(&info0)) != 0) {
...
static hi_s32 sample_gfbg_double_buf_mode(vo_device_info *vo_dev_info)
{
    hi_s32 ret;
    pthread_gfbg_sample_info info0;
    g_vo_intf_type = vo_dev_info->hi_vo_intf_type;
    ...
    if (pthread_create(&g_gfbg_thread, 0, sample_gfbg_refresh, (hi_void*)(&info0)) != 0) {
...
static hi_s32 sample_gfbg_one_buf_mode(vo_device_info *vo_dev_info)
{
    hi_s32 ret;
    pthread_gfbg_sample_info info0;
    g_vo_intf_type = vo_dev_info->hi_vo_intf_type;
...
static hi_s32 sample_gfbg_none_buf_mode(vo_device_info *vo_dev_info)
{
    hi_s32 ret;
    pthread_gfbg_sample_info info0;
    g_vo_intf_type = vo_dev_info->hi_vo_intf_type;
...
static hi_s32 sample_gfbg_clut_mode(vo_device_info *vo_dev_info)
{
    hi_s32  ret;
    pthread_gfbg_sample_info info0;
    g_vo_intf_type  = vo_dev_info->hi_vo_intf_type;
...
static hi_void gfbg_init_surface(hi_tde_surface *src_surface, hi_tde_surface *dst_surface,
...
static hi_s32 gfbg_draw_by_tde(hi_tde_surface *src_surface, hi_tde_surface *dst_surface,
...
static hi_s32 sample_gfbg_rotation(vo_device_info *vo_dev_info)
{
    hi_s32 ret;
    hi_vb_cfg vb_conf;
...
static hi_void sample_gfbg_to_exit_signal(hi_void)
{
    ...
    if (g_canvas_phy) {
        hi_mpi_sys_mmz_free(g_canvas_phy, HI_NULL);
        g_canvas_phy = 0;
    }

    if (g_picture_phy) {
        hi_mpi_sys_mmz_free(g_picture_phy, HI_NULL);
        g_picture_phy = 0;
    }
    return;
}
...
int app_main(int argc, char *argv[])
{
    ...
    vo_dev_info.hi_vo_intf_type = HI_VO_INTF_BT1120; /* default: BT1120 */
...
#else
int sample_gfbg_main(int argc, char *argv[])
{
    hi_s32 ret;
    vo_device_info vo_dev_info;
    vo_dev_info.vo_dev = SAMPLE_VO_DEV_DHD0;
