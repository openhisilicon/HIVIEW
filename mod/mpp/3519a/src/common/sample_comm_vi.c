...
HI_S32 SAMPLE_COMM_VI_GetComboAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, combo_dev_t MipiDev, combo_dev_attr_t* pstComboAttr)
{
    switch (enSnsType)
    {
        ...
        case SONY_IMX290_MIPI_2M_30FPS_12BIT_WDR2TO1:
            hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_4lane_CHN0_SENSOR_IMX290_12BIT_2M_WDR2to1_ATTR, sizeof(combo_dev_attr_t));
            break;

        case SONY_IMX290_SLAVE_MIPI_2M_60FPS_10BIT:
            if (0 == MipiDev)
            {
                hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN0_SENSOR_IMX290_SLAVE_10BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            }
            else if (1 == MipiDev)
            {
                hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN1_SENSOR_IMX290_SLAVE_10BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            }
            else if (2 == MipiDev)
            {
                hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN2_SENSOR_IMX290_SLAVE_10BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            }
            else if (3 == MipiDev)
            {
                hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN3_SENSOR_IMX290_SLAVE_10BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            }
            else if (4 == MipiDev)
            {
                hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN4_SENSOR_IMX290_SLAVE_10BIT_2M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            }
            else
            {
                SAMPLE_PRT("unsupported mipi dev :%d for SnsType :%d\n", MipiDev, enSnsType);
        ...
        case SONY_IMX377_MIPI_8M_30FPS_10BIT:
            hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_4lane_CHN0_SENSOR_IMX377_10BIT_8M_NOWDR_ATTR, sizeof(combo_dev_attr_t));
            break;

       case NVP6324_MIPI_2M_30FPS_8BIT:
            hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_4lane_CHN0_NVP6324_2M_SP420_ATTR, sizeof(combo_dev_attr_t));
            break;
...
HI_S32 SAMPLE_COMM_VI_GetDevAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_DEV_ATTR_S* pstViDevAttr)
{
    switch (enSnsType)
    {
        ...
        case SONY_IMX377_MIPI_8M_30FPS_10BIT:
            hi_memcpy(pstViDevAttr, sizeof(VI_DEV_ATTR_S), &DEV_ATTR_IMX377_8M_BASE, sizeof(VI_DEV_ATTR_S));
            break;

        case NVP6324_MIPI_2M_30FPS_8BIT:
            hi_memcpy(pstViDevAttr, sizeof(VI_DEV_ATTR_S), &DEV_ATTR_MIPI_SP420_2M_BASE, sizeof(VI_DEV_ATTR_S));
            break;
...
HI_S32 SAMPLE_COMM_VI_GetPipeAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_PIPE_ATTR_S* pstPipeAttr)
{
    ...
    switch (enSnsType)
    {
        ...
        case NVP6324_MIPI_2M_30FPS_8BIT:
            hi_memcpy(pstPipeAttr, sizeof(VI_PIPE_ATTR_S), &PIPE_ATTR_1920x1080_SP420_3DNR_RFR, sizeof(VI_PIPE_ATTR_S));
            break;
...
HI_S32 SAMPLE_COMM_VI_StartViPipe(SAMPLE_VI_INFO_S* pstViInfo)
{
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            {
                printf("%s => ViPipe:%d, stPipeAttr.enPixFmt:%d\n", __func__, ViPipe, stPipeAttr.enPixFmt);
                s32Ret = HI_MPI_VI_CreatePipe(ViPipe, &stPipeAttr);
                if (s32Ret != HI_SUCCESS)
                {
                    SAMPLE_PRT("HI_MPI_VI_CreatePipe failed with %#x!\n", s32Ret);
...
HI_VOID SAMPLE_COMM_VI_GetSensorInfo(SAMPLE_VI_CONFIG_S* pstViConfig)
{
    ...
    printf("%s => SENSOR0_TYPE:%d, SENSOR1_TYPE:%d\n", __func__, SENSOR0_TYPE, SENSOR1_TYPE);
}
