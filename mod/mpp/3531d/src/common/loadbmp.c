 ...
 * Description: Common function for loading bmp data.
...
static hi_void updata_osd_logo_size_info(const osd_bit_map_info *bmp_info, osd_color_format fmt, osd_logo *video_logo)
{
    ...
    switch (fmt) {
        ...
        case OSD_COLOR_FORMAT_CLUT2:
            video_logo->stride = video_logo->width / PIX_PER_BYTTE_CLUT2;
            return;
        case OSD_COLOR_FORMAT_CLUT4:
            video_logo->stride = video_logo->width / PIX_PER_BYTTE_CLUT4;
            return;
        default:
    ...
    const bmp_data_size_info *data_size_info, osd_logo *video_logo)
{
    ...
    if (fmt == OSD_COLOR_FORMAT_CLUT4) {
        width /= PIX_PER_BYTTE_CLUT4;
    } else if (fmt == OSD_COLOR_FORMAT_CLUT2) {
        width /= PIX_PER_BYTTE_CLUT2;
    } else {
    }
...
static hi_s32 copy_2byte_bmp(hi_u16 bpp, osd_color_format fmt,
    const hi_u8 *data, const bmp_data_size_info *data_size_info, osd_logo *video_logo)
{
    hi_u32 i, j;
    hi_u8 *rgb_buf = video_logo->rgb_buf;
    hi_u8 r, g, b;
...
static hi_s32 copy_4byte_bmp(hi_u16 bpp, const hi_u8 *data,
    const bmp_data_size_info *data_size_info, osd_logo *video_logo)
{
    hi_u32 i, j;
...
static hi_s32 copy_original_bmp_data_and_fill_alpha(hi_u16 bpp, osd_color_format fmt,
    const hi_u8 *data, const bmp_data_size_info *data_size_info, osd_logo *video_logo)
{
    switch (fmt) {
        case OSD_COLOR_FORMAT_RGB444:
    ...
    FILE *file, osd_logo *video_logo, osd_color_format fmt)
{
    ...
    if (bpp == 0) {
        if (fmt == OSD_COLOR_FORMAT_CLUT2) {
            data_size_info.stride = data_size_info.width / PIX_PER_BYTTE_CLUT2;
        } else if (fmt == OSD_COLOR_FORMAT_CLUT4) {
            data_size_info.stride = data_size_info.width / PIX_PER_BYTTE_CLUT4;
        } else {
...
hi_s32 load_bmp_canvas(const hi_char *filename, osd_logo *video_logo, osd_color_format fmt)
{
    ...
    /* height > 0 */
    if (bmp_info.bmp_header.bi_height > video_logo->height || bmp_info.bmp_header.bi_width > video_logo->width) {
        printf("Bitmap's size(%u, %u) is bigger than canvas's size(%u, %u). Load bitmap error!\n",
            bmp_info.bmp_header.bi_height, video_logo->height, bmp_info.bmp_header.bi_width, video_logo->width);
        goto read_bmp_failed;
    }
