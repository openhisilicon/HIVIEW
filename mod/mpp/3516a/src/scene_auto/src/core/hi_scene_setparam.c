...
HI_S32 HI_SCENE_SetDynamicVideoGamma_AutoGenerate(VI_PIPE ViPipe, HI_U64 u64Exposure, HI_U64 u64LastExposure, HI_U8 u8Index)
{
    ...
    HI_U32 i, j = 0;
    HI_U32 u32ExpLevel = 0;
    static HI_U32 u32LastExpLevel[HI_SCENE_PIPETYPE_NUM];
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    if (u64Exposure != u64LastExposure)
    {
        for (i = 0; i < g_astScenePipeParam[u8Index].stDynamicGamma.u32InterVal; i++)
        {
            ...
            {
                stIspGammaAttr.u16Table[j] = SCENE_TimeFilter(g_astScenePipeParam[u8Index].stDynamicGamma.au16Table[u32LastExpLevel[ViPipe]][j],
        ...
        u32LastExpLevel[ViPipe] = u32ExpLevel;
    }
...
HI_S32 HI_SCENE_SetDynamicAE_AutoGenerate(VI_PIPE ViPipe, HI_U64 u64Exposure, HI_U64 u64LastExposure, HI_U8 u8Index)
{
    ...
    if(stSmartExpAttr.bEnable)
    {
        ...
        HI_MPI_ISP_GetSmartInfo(ViPipe, &stSmartInfo);
        
        if(stSmartInfo.stROI[0].bEnable)
        {          
            if(stIspExpInfo.u32ISO > 150)
            {
               stSmartExpAttr.u16ExpCoefMax = SCENE_Interpulate(stIspExpInfo.u32ISO, 200, 1600, 2000, 1024);
               stSmartExpAttr.u16ExpCoefMin = SCENE_Interpulate(stIspExpInfo.u32ISO, 200, 512, 2000, 400);
            }
            else if(stIspExpInfo.u32Exposure > 10000)
            {
                ...
                stSmartExpAttr.u16ExpCoefMin = SCENE_Interpulate(stIspExpInfo.u32Exposure, 10000, 650, 90000, 512);
            }
            else
            {
                stSmartExpAttr.u16ExpCoefMax = SCENE_Interpulate(stIspExpInfo.u32Exposure, 2000, 1600, 10000, 2048);
                stSmartExpAttr.u16ExpCoefMin = SCENE_Interpulate(stIspExpInfo.u32Exposure, 2000, 750, 10000, 650); 
            }  
...
HI_S32 HI_SCENE_SetRCParam_AutoGenerate(VI_PIPE ViPipe,HI_U8 u8Index)
{
    ...
    PAYLOAD_TYPE_E  enType;
    static HI_U32  u32IsoInfo[HI_SCENE_PIPETYPE_NUM][8];
    static HI_U32  u32MeanQpInfo[HI_SCENE_PIPETYPE_NUM][8];
    HI_U32  u32PicWidth;
    HI_U32  u32PicHeight;
    HI_U32  u32Index = 0;

    memset(u32IsoInfo,   100, sizeof(u32IsoInfo));
    memset(u32MeanQpInfo, 30, sizeof(u32MeanQpInfo));
    ...
    u32MinQP = u32MeanQpInfo[ViPipe][0];
    ...
    for (i = 0; i < 7; i++)
    {
	 u32IsoInfo[ViPipe][i]= u32IsoInfo[ViPipe][i+1];
	 u32MeanQpInfo[ViPipe][i]=u32MeanQpInfo[ViPipe][i+1];
    }
    u32IsoInfo[ViPipe][i]= u32Iso;
    u32MeanQpInfo[ViPipe][i]=u32MeanQp;

    for (i=0; i< 8; i++)
    {
        if(u32MinQP < u32MeanQpInfo[ViPipe][i]) u32MinQP = u32MeanQpInfo[ViPipe][i];
        u32SumMeanQP += u32MeanQpInfo[ViPipe][i];
	    u32SumIso += u32IsoInfo[ViPipe][i];
    }
