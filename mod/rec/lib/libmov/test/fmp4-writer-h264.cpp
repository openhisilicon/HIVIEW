...
static void h264_read_frame(fmp4_writer_t* mov, int width, int height, const uint8_t* ptr, const uint8_t* end)
{
	int64_t pts = 0;
	int track = -1;
	bool spspps = false;
	...
	while (nalu < end)
	{
        ...
        if(nal_unit_type <= 5)
        {
    			...
    			if (!spspps)
    			{
    				...
    				track = fmp4_writer_add_video(mov, MOV_OBJECT_H264, width, height, extra_data, extra_data_size);
    			}
...
void fmp4_writer_h264(const char* h264, int width, int height, const char* mp4)
{
	long bytes = 0;
