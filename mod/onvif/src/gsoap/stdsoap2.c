...
#endif
#ifndef WITH_LEAN
	//printf("[%s]2222222222: omode: %d, peerlen: %d\n",__FILE__, soap->omode, soap->peerlen);
      ...
      { if (soap->peerlen)
          ...
          nwritten = sendto(sk, (char*)s, (SOAP_WINSOCKINT)n, soap->socket_flags, (struct sockaddr*)&soap->peer, (SOAP_WINSOCKINT)soap->peerlen);
			...
			//printf("[%s]sendto :sk: %d, rwlen: %d, ip: %s, port: %d\n",__FILE__, sk, nwritten, inet_ntoa((soap->peer).sin_addr), ntohs((soap->peer).sin_port));
	  	}
	  	else
	  	{
          nwritten = send(sk, s, (SOAP_WINSOCKINT)n, soap->socket_flags);
		  //printf("[%s]--->>>4444444444444:sk: %d, rwlen: %d\n",__FILE__, sk, nwritten);
        }
     ...
     char *_s = s + n;
     *_s = '\0';
     printf("[%s]send: omode: %d, peerlen: %d, s: %s\n",__FILE__, soap->omode, soap->peerlen, s);
     #endif
