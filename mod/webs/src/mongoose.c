...
#if MG_ENABLE_BROADCAST
#warning ">>>>> MG_ENABLE_BROADCAST"
void mg_broadcast(struct mg_mgr *mgr, mg_event_handler_t cb, void *data,
                  size_t len) {
  struct ctl_msg ctl_msg;
  //printf(">>>>>>>>> len:%d, mgr->ctl[0]:%d, data:%p\n", len, mgr->ctl[0], data);
  /*
  ...
  if (mgr->ctl[0] != INVALID_SOCKET && data != NULL &&
    ...
    size_t dummy;
    //
    //printf("maohw send mgr->ctl[0]:%d len:%d, sess:%u\n", mgr->ctl[0], len, *((int*)data));
    ctl_msg.magic = 0x55aa55aa;
    ctl_msg.callback = cb;
    ...
    //printf("maohw sned2 mgr->ctl[0]:%d len:%d, sess:%u\n", mgr->ctl[0], len, *((int*)data));
...
static void mg_mgr_handle_ctl_sock(struct mg_mgr *mgr) {
  struct ctl_msg ctl_msg;
  //
  //printf("maohw read from mgr->ctl[1]: %d ....... \n", mgr->ctl[1]);
  int len =
      (int) MG_RECV_FUNC(mgr->ctl[1], (char *) &ctl_msg, sizeof(ctl_msg), 0);
  size_t dummy = MG_SEND_FUNC(mgr->ctl[1], ctl_msg.message, 1, 0);
  assert(ctl_msg.magic == 0x55aa55aa);
  //printf("maohw read from mgr->ctl[1] %d, len:%d\n", mgr->ctl[1], len);
  
  (void) dummy; /* https://gcc.gnu.org/bugzilla/show_bug.cgi?id=25509 */
  if (len >= (int) sizeof(ctl_msg.callback) && ctl_msg.callback != NULL) {
    struct mg_connection *nc;
    for (nc = mg_next(mgr, NULL); nc != NULL; nc = mg_next(mgr, nc)) {
      //printf("maohw CB, nc:%p, nc->user_data:%p\n", nc, nc->user_data);
...
#if MG_ENABLE_BROADCAST
#warning "MG_ENABLE_BROADCAST pair."
  if(mg_socketpair(iface->mgr->ctl, SOCK_DGRAM) < 0)
  //maohw if(mg_socketpair(iface->mgr->ctl, SOCK_STREAM) < 0)
  {
    printf("mg_socketpair err.\n");
  }
  printf("mg_socketpair ok.\n");
#endif
}
