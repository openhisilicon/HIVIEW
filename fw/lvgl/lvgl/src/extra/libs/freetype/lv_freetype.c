...
static FT_Stroker stroker;
    ...
    static FT_Glyph outline_glyph = NULL;
    static FT_BitmapGlyph outline_bitmap = NULL;
...
bool lv_freetype_init(uint16_t max_faces, uint16_t max_sizes, uint32_t max_bytes)
{
    ...
    if(error) {
        LV_LOG_ERROR("init freeType error(%d)");
        return false;
    }

    lv_ll_init(&names_ll, sizeof(name_refer_t));
...
Fail:
    ...
    LV_UNUSED(max_sizes);
    LV_UNUSED(max_bytes);
    lv_ll_init(&face_control.face_ll, sizeof(FT_Face *));
    return true;
...
static bool get_outline_glyph(const lv_font_t * font, FT_Face face,
                           FT_UInt glyph_index, lv_font_glyph_dsc_t * dsc_out)
{
    int ret = 0;
    if(FT_Load_Glyph(face, glyph_index, FT_LOAD_DEFAULT)) {
    ...
    if(outline_glyph)
    {
      //printf("FT_Done_Glyph outline_glyph:%p\n", outline_glyph);
      FT_Done_Glyph(outline_glyph);
    }
    
    ret = FT_Get_Glyph(face->glyph, &outline_glyph);

    FT_Glyph_StrokeBorder(&outline_glyph, stroker, 0, 1);
    ret = FT_Glyph_To_Bitmap(&outline_glyph, FT_RENDER_MODE_NORMAL, 0, 1);
    outline_bitmap = (FT_BitmapGlyph)outline_glyph;
    
    dsc_out->adv_w = (outline_bitmap->root.advance.x >> 16);
    dsc_out->box_h = outline_bitmap->bitmap.rows;         /*Height of the bitmap in [px]*/
    dsc_out->box_w = outline_bitmap->bitmap.width;        /*Width of the bitmap in [px]*/
    dsc_out->ofs_x = outline_bitmap->left;                /*X offset of the bitmap in [pf]*/
    dsc_out->ofs_y = outline_bitmap->top -
                     outline_bitmap->bitmap.rows;         /*Y offset of the bitmap measured from the as line*/
    dsc_out->bpp = 8;         /*Bit per pixel: 1/2/4/8*/
    return true;
}
                                   ...
                                   lv_font_glyph_dsc_t * dsc_out, uint32_t unicode_letter, uint32_t unicode_letter_next)
{
    ...
    if(dsc->style & FT_FONT_STYLE_BOLD) {
        ...
        goto end;
    } else if(dsc->style & FT_FONT_STYLE_OUTLINE) {
        if(!get_outline_glyph(font, face, glyph_index, dsc_out)) {
            return false;
        }
        goto end;
    }
...
static const uint8_t * get_glyph_bitmap_cb_cache(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    {
      if(outline_bitmap) {
        return (const uint8_t *)(outline_bitmap->bitmap.buffer);
      }
      return NULL;
    }  
...
static FT_Face face_find_in_list(lv_ft_info_t * info)
{
    ...
    FT_Face * pface = lv_ll_get_head(&face_control.face_ll);
    while(pface) {
        dsc = (lv_font_fmt_ft_dsc_t *)(*pface)->generic.data;
        if(strcmp(dsc->name, info->name) == 0) {
            return *pface;
        }
        pface = lv_ll_get_next(&face_control.face_ll, pface);
    }
...
{
    FT_Face * pface;
    pface = (FT_Face *)lv_ll_ins_tail(&face_control.face_ll);
    *pface = face;
}
...
static void face_remove_from_list(FT_Face face)
{
    ...
    while(pface) {
        if(*pface == face) {
            lv_ll_remove(&face_control.face_ll, pface);
            lv_mem_free(pface);
            break;
        }
        pface = lv_ll_get_next(&face_control.face_ll, pface);
                                     ...
                                     lv_font_glyph_dsc_t * dsc_out, uint32_t unicode_letter, uint32_t unicode_letter_next)
{
    ...
    if(dsc->style & FT_FONT_STYLE_OUTLINE)
    {
      return get_outline_glyph(font, face, glyph_index, dsc_out);
    }  
...
static bool lv_ft_font_init_nocache(lv_ft_info_t * info)
{
    ...
    if(face == NULL) {
        ...
        if(error) {
            LV_LOG_WARN("create face error(%d)");
            goto Fail;
        }
...
static const char * name_refer_find(const char * name)
{
    name_refer_t * refer = lv_ll_get_head(&names_ll);
    while(refer) {
        if(strcmp(refer->name, name) == 0) {
            refer->cnt += 1;
            return refer->name;
        }
        refer = lv_ll_get_next(&names_ll, refer);
    }
    return NULL;
}
...
static void name_refer_del(const char * name)
{
    ...
    LV_LOG_WARN("name_in_names_del error(not find:%p).");
}
...
static const char * name_refer_save(const char * name)
{
    ...
    name_refer_t * refer = lv_ll_ins_tail(&names_ll);
    if(refer) {
